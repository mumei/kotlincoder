...
class KtFunction @JvmOverloads constructor(
        val name:String,
    ...
    override fun print(parent: PrintCode?, codeCreator: CodeCreator): CodeCreator {
        ...
        codeCreator.put("${if(modifiers.isNotEmpty())"${modifiers.joinToString(separator = " ")} " else ""}fun $name (")
        ...
        codeCreator.intentIndex--
        if(clazz==null||clazz.name=="void"){
            if(parent is KtInterface){
                codeCreator.put(")")
                return codeCreator
            }else{
                codeCreator.put("){")
